        ...
        try {
            for (mdFieldPosition = 0; mdFieldPosition < mdField.length(); mdFieldPosition++) { //Loop through MD field
                ...
                if (mdField.substring(mdFieldPosition, mdFieldPosition + 1).matches("[AGCTagct]")) { //A letter indicates a polymorphism
                    tagSequencePosition += Integer.parseInt(matchingBaseCount)+1; //If we came here from a number, move tag pointer
                ...
                if (mdField.substring(mdFieldPosition, mdFieldPosition + 1).matches("[0-9]")) {
                    if (deletion) {
                        refHaplotype = refHaplotype.concat(deletionLength+"-"); //A number during a deletion indicates a deletion from the reference...
                        tagHaplotype = tagHaplotype.concat(deletionLength+"+"); //...Or alternatively, an insertion in the tag
                        variantPosition[variantCount] = Integer.toString(tagSequencePosition);
                        variantCount++;
                        deletionLength = 0;
                        deletion = false;
