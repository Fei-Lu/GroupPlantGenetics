...
//        Collections.sort(files1, comparator);
     ...
     * allele count file
     */
    public static void getAlleleCount(List<File> files, String sample2PopFile, String alleleCountDir){
        Map<String, String> sample2PopMap = RowTableTool.getMap(sample2PopFile, 1 ,0, false);
        ...
        IntStream.range(0, files.size()).parallel().forEach(e->{
            try (BufferedReader br = IOTool.getReader(files.get(e));
                ...
                String[] pops = sample2PopMap.values().stream().distinct().toArray(String[]::new);
                ...
                StringBuilder sb = new StringBuilder();
                sb.append("CHROM\tPOS\tSNPID\t").append(String.join("\t", pops));
                bw.write(sb.toString());
                bw.newLine();
                ...
                while ((line = br.readLine())!=null){
                    ...
                    sb.setLength(0);
                    sb.append(String.join("\t", temp.subList(0, 3))).append("\t");
                    for (String population: pops){
                        indexes = pop2IndexesMap.get(population);
                        refAlleleCount = 0;
                        altAlleleCount = 0;
    ...
    public static void getAlleleCount(String genotypeDir, String sample2PopFile_AB, String sample2PopFile_D,
        ...
        Predicate<File> ab = file -> SubgenomeCombination.AB.containChrID(Integer.parseInt(file.getName().substring(3,6)));
        List<File> abFiles = files.stream().filter(ab).collect(Collectors.toList());
        List<File> dFiles = files.stream().filter(ab.negate()).collect(Collectors.toList());
        getAlleleCount(abFiles, sample2PopFile_AB, alleleCountDir);
        getAlleleCount(dFiles, sample2PopFile_D, alleleCountDir);
    }
}
