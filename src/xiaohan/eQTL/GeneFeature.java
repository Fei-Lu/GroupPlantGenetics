...
//        this.readFile(infileS);
        ...
        try {
            ...
            for (int i = 0; i < geneNumber; i++) {
                ...
                for (int j = 0; j < transcriptNumber; j++) {
                    ...
                    Transcript t = new GeneFeature.Transcript(tem[1], chr, Integer.valueOf(tem[3]), Integer.valueOf(tem[4]), Byte.valueOf(tem[5]));
                    tem = br.readLine().split("\t");
     ...
     * @return
     */
    public ChrPos getTESOfGene (int geneIndex) {
        int index = this.getLongestTranscriptIndex(geneIndex);
        List<Range> utr5 = this.get5UTRList(geneIndex, index);
        if (utr5.isEmpty()) return null;
        if (this.getTranscriptStrand(geneIndex, index) == 1) {
    ...
    public int getGeneIndex (int chr, int pos) {
        ...
        GeneFeature.Gene query = new GeneFeature.Gene(chr, pos, pos+1);
    ...
    public int getGeneIndex (String geneName) {
        ...
        return Arrays.binarySearch(genes, new GeneFeature.Gene(geneName));
    }
    ...
    public int getGeneStartIndexOfChromosome(int chromosome) {
        ...
        GeneFeature.Gene query  = new GeneFeature.Gene("Query", chromosome, Integer.MIN_VALUE, Integer.MIN_VALUE, Byte.MIN_VALUE, "", "");
    ...
    public int getGeneEndIndexOfChromosome(int chromosome) {
        ...
        GeneFeature.Gene query  = new GeneFeature.Gene("Query", chromosome+1, Integer.MIN_VALUE, Integer.MIN_VALUE, Byte.MIN_VALUE, "", "");
        int hit  = Arrays.binarySearch(genes, query);
        int index = -hit-1;
    ...
    public void readFromWheatGFF (String infileS) {
        try {
            ...
            genes = new GeneFeature.Gene[geneNames.length];
            ...
            for (int i = 0; i < info.length; i++) {
                ...
                if (tem[2].startsWith("gene")) {
                    ...
                    genes[index] = new GeneFeature.Gene(query, Integer.valueOf(tem[0]), Integer.valueOf(tem[3]), Integer.valueOf(tem[4])+1, (byte)(tem[6].equals("+")? 1:0), biotype, description);
                }
            }
            for (int i = 0; i < info.length; i++) {
                tem = info[i].split("\t");
                if (tem[2].startsWith("mRNA")) {
                    ...
                    GeneFeature.Transcript t = new GeneFeature.Transcript(te[0].split("=")[1], Integer.valueOf(tem[0]), Integer.valueOf(tem[3]), Integer.valueOf(tem[4])+1, (byte)(tem[6].equals("+")? 1:0));
    ...
    class Gene implements Comparable<GeneFeature.Gene> {
        String geneName = null;
        Range geneRange = null;
        byte strand = Byte.MIN_VALUE;
        String biotype = null;
        String description = null;
        ArrayList<GeneFeature.Transcript> ts = new ArrayList();
        ...
        public void addTranscript (GeneFeature.Transcript t) {
            ts.add(t);
        }
        ...
        public int getTranscriptIndex (String transcriptName) {
            return Collections.binarySearch(ts, new GeneFeature.Transcript(transcriptName));
        }
        ...
        @Override
        public int compareTo(GeneFeature.Gene t) {
            if (sortType == 0) {
     ...
     */
    class Transcript implements Comparable<GeneFeature.Transcript> {
        ...
        @Override
        public int compareTo(GeneFeature.Transcript t) {
