...
/**
 * @ author: yxh
 * @ created: 2021-07-01 : 8:05 PM
 */

/**
 * @author Daxing Xu
 */
public class LibraryOfGRTcopy extends LibraryInfo {

    public LibraryOfGRTcopy(String barcodeFileS, String libFastqMapFileS, String cutter1, String cutter2) {
...
//            this.splitBarcode(libs[i], fastqR1, fastqR2, barcodeR1TaxaMap, barcodeR2TaxaMap, taxaNames, outputDir, cutter1, cutter2);
            this.split(libs[i], fastqR1, fastqR2, barcodeR1TaxaMap, barcodeR2TaxaMap, taxaNames, outputDir, cutter1, cutter2);
...
//                if (enzymeIndex2.isEmpty()) continue;
    ...
    private void split(String lib, String fastqR1, String fastqR2,
                        HashMap<String, Set<String>> barcodeR1TaxaMap, HashMap<String, Set<String>> barcodeR2TaxaMap, String[] taxaNames, String outputDir, String cutter1, String cutter2) {
        long start = System.nanoTime();
        HashMap<String, BufferedWriter[]> taxaWriterMap = new HashMap<>();
        BufferedWriter bw;
...
//            taxaWriterMap.put(str, bwArray);
//        }
        bw = IOUtils.getTextWriter(new File(outputDir, "results.txt").getAbsolutePath());
        try {
            BufferedReader br1, br2;
            if (fastqR1.endsWith(".gz")) {
                br1 = IOUtils.getTextGzipReader(fastqR1);
            ...
            while ((read1ID = br1.readLine()) != null) {
                ...
                enzymeIndex1 = StringTool.getIndexOfSubStr(substr1, "GGCCGC");
                enzymeIndex2 = StringTool.getIndexOfSubStr(substr2, "CGG");
                if (enzymeIndex1.isEmpty() && enzymeIndex2.isEmpty()) {
                    EnzymeNotCnt++;
                } else if (!enzymeIndex1.isEmpty() && enzymeIndex2.isEmpty()) {
                    Enzyme1Cnt++;
                } else if (!enzymeIndex2.isEmpty() && enzymeIndex1.isEmpty()) {
                    Enzyme2Cnt++;
                } else EnzymeBothCnt ++;
//                if (enzymeIndex1.isEmpty() && enzymeIndex2.isEmpty()) continue;
//                barcode1 = substr1.substring(0, enzymeIndex1.get(0));
...
//                bwArray[1].newLine();
            ...
            bw.write("Enzyme1Cnt\tEnzyme2Cnt\tEnzymeBothCnt\tEnzymeNotCnt\n");
            bw.write(Enzyme1Cnt+"\t"+Enzyme2Cnt+"\t"+EnzymeBothCnt+"\t"+EnzymeNotCnt+"\n");
            bw.flush();
            bw.close();
            br1.close();
